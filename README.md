...
- **原生会话恢复**：Codex thread 和 Claude Code session 分别保存，切换回来会调用各自 CLI 的原生 resume。
- **持久任务边界**：消息先进入 durable inbox；Agent 已完成但回复尚未送达时只补发结果，不会重新执行任务。
- **冻结执行配置**：Agent、工作区、access、模型和思考级别在任务入队时冻结，之后的设置修改不会改变已排队任务。
- **可靠收发**：轮询失败会退避重试，未送达回复进入 outbox，待下一次有效上下文到来时补发。
- **显式产物外发**：附件会安全缓存；Agent 产物必须先经 `/artifacts` 查看，再由微信中的 `/send` 明确发送，不会因模型在回复中提到路径而自动外发。
